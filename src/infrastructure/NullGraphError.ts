/**
 * Error thrown when canvas or node code requires an {@link LGraph} reference that is missing.
 *
 * {@link LGraphCanvas} and related interaction handlers throw this when `graph` is `null` or
 * `undefined` at a point where graph operations are mandatory (rendering, hit-testing, linking,
 * etc.). Callers should ensure a graph is assigned via {@link LGraphCanvas.setGraph} before use.
 * @see {@link LGraphCanvas.graph}
 */
