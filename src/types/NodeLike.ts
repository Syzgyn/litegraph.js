 ...
 *
 * Implemented by `LGraphNode`, subgraph boundary nodes, and other graph entities that
...
export interface NodeLike {
  ...
  /**
   * Determines whether a link from `fromSlot` on `node` may connect to `toSlot` on this node.
   ...
   * @param fromSlot The output slot on `node` that would send the link.
