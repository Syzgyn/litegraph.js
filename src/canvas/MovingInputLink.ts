 ...
 *
 * Created by {@link LinkConnector.moveInputLink} when the user begins dragging a link away from
 * an input slot. The output side of the link remains fixed; only the input side is repositioned
 * on drop.
 * @remarks
 * This is the input-side counterpart to {@link MovingOutputLink}. Instances are short-lived and
...
export class MovingInputLink extends MovingLinkBase {
   ...
   *
   * Matches {@link MovingLinkBase.outputSlot}. Exposed here as `fromSlot` to satisfy the
   ...
   * otherwise it falls back to {@link MovingLinkBase.outputPos}.
   */
   ...
   *
   * Always {@link LinkDirection.NONE} for input-side moves, because the free end of the link
   ...
   *
   * Delegates to {@link LGraphNode.canConnectTo}, passing this link's fixed output node and slot
   * as the upstream source. Also accepts {@link SubgraphIO} slots when dropping onto subgraph
   * boundary nodes.
   ...
   * input connection is removed and {@link LGraphNode.connectSlots} creates or updates the
   * connection while preserving reroute parentage via {@link LLink.parentId}.
   ...
   *
   * Used when the user drops a moving input link onto a {@link SubgraphOutput}. Creates a new
   ...
   * The reroute being dropped on is parented to {@link MovingLinkBase.fromReroute}, orphaned
   * reroutes in the original chain are cleaned up, and {@link LGraphNode.connectSlots} completes
   * the connection while retaining any downstream reroute chain via {@link LLink.parentId}.
