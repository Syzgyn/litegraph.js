...
/**
 * A placeholder subgraph input slot that materialises a real `SubgraphInput` on first connection.
 *
 * Rendered at the bottom of the `SubgraphInputNode` slot list. When the user drags a link
 ...
 * inputs. See `SubgraphInputNode.emptySlot`.
 * @see `EmptySubgraphOutput`
...
export class EmptySubgraphInput extends SubgraphInput {
   ...
   *
   * Derives a unique name from the target input slot, calls `Subgraph.addInput`, then
   * delegates to the new `SubgraphInput.connect`.
   ...
   * of concrete `SubgraphInput` slots.
   */
