...
export class SliderWidget extends BaseWidget<ISliderWidget> implements ISliderWidget {
  ...
  override drawWidget(ctx: CanvasRenderingContext2D, {
    ...
    // Draw slider bar
    ctx.fillStyle = this.options.sliderColor ?? "#678"
    ...
    if (this.marker != null) {
      let markerNValue = (this.marker - this.options.min) / range
      markerNValue = clamp(markerNValue, 0, 1)
      ctx.fillStyle = this.options.markerColor ?? "#AA9"
      ctx.fillRect(
        margin + markerNValue * (width - margin * 2),
        y,
        2,
        height,
      )
