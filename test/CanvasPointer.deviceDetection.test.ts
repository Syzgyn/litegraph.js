...
describe("CanvasPointer Device Detection - Efficient Timestamp-Based TDD Tests", () => {
  ...
  describe("Initial State", () => {
    test("should start in mouse detected mode immediately after loading", () => {
      expect(pointer.detectedDevice).toBe("mouse")
    })

    test("should have no last wheel event time immediately after loading", () => {
      expect(pointer.lastWheelEventTime).toBe(0)
      expect(pointer.hasReceivedWheelEvent).toBe(false)
    })

    test("should have no buffered Linux event immediately after loading", () => {
  ...
  describe("First Event Detection", () => {
    describe("switching to trackpad on first event", () => {
      test("should switch to trackpad if first event is pinch-to-zoom with deltaY < 10", () => {
      ...
      test("should switch to trackpad if first event is pinch-to-zoom with deltaY = 9.999", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: true,
          deltaY: 9.999,
          deltaX: 0,
      ...
      test("should NOT switch to trackpad if first event is pinch-to-zoom with deltaY = 10", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: true,
          deltaY: 10,
          deltaX: 0,
      ...
      test("should switch to trackpad if first event is two-finger panning with integer values", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: false,
          deltaY: 5,
          deltaX: -3,
      ...
      test("should switch to trackpad if first event is two-finger panning with ctrlKey true", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: true,
          deltaY: 7,
          deltaX: 4,
      ...
      test("should switch to trackpad if first event is negative pinch-to-zoom with deltaY > -10", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: true,
          deltaY: -9.5,
          deltaX: 0,
    ...
    describe("remaining in mouse mode on first event", () => {
      test("should remain in mouse mode if first event is pinch-to-zoom with deltaY >= 10", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: true,
          deltaY: 10.1,
          deltaX: 0,
      ...
      test("should remain in mouse mode if first event is mouse wheel with deltaY = 120", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: false,
          deltaY: 120,
          deltaX: 0,
      ...
      test("should remain in mouse mode if first event has only deltaY (no deltaX)", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: false,
          deltaY: 30,
          deltaX: 0,
  ...
  describe("Mode Switching from Mouse to Trackpad", () => {
    ...
    test("should switch to trackpad on two-finger panning with non-zero deltaX and deltaY", () => {
    ...
    test("should NOT switch to trackpad on two-finger panning with zero deltaX", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT switch to trackpad on two-finger panning with zero deltaY", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should switch to trackpad on pinch-to-zoom with deltaY < 10", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should switch to trackpad on pinch-to-zoom with deltaY = -5.5", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT switch to trackpad on pinch-to-zoom with deltaY = 10", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
  ...
  describe("Mode Switching from Trackpad to Mouse", () => {
    ...
    test("should switch to mouse on clear mouse wheel event with deltaY > 80", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should switch to mouse on clear mouse wheel event with deltaY = 120", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should switch to mouse on clear mouse wheel event with negative deltaY < -80", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT switch to mouse with deltaY = 80", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT switch to mouse with deltaY = 79.999", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
  ...
  describe("500ms Cooldown Period", () => {
    test("should NOT allow switching from mouse to trackpad within 500ms", () => {
      pointer.detectedDevice = "mouse"
    ...
    test("should allow switching from mouse to trackpad after 500ms", () => {
      pointer.detectedDevice = "mouse"
    ...
    test("should NOT allow switching from trackpad to mouse within 500ms", () => {
      pointer.detectedDevice = "trackpad"
    ...
    test("should maintain cooldown even with multiple events", () => {
      pointer.detectedDevice = "mouse"
      ...
      for (const [index, eventData] of trackpadEvents.entries()) {
  ...
  describe("Linux Wheel Event Buffering", () => {
    ...
    test("should buffer possible Linux wheel event and create single timeout", () => {
    ...
    test("should reuse timer when buffering new Linux event", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
    ...
    test("should buffer negative Linux wheel values", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT buffer event with deltaY < 10", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT buffer event with non-zero deltaX", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should switch to mouse if follow-up event has same deltaY within 10ms", () => {
      const clearTimeoutSpy = vi.spyOn(global, "clearTimeout")
    ...
    test("should switch to mouse if follow-up event is divisible by original deltaY within 10ms", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should switch to mouse if follow-up deltaY is divisible by original (base 15)", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should switch to mouse if original deltaY is divisible by follow-up", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT switch to mouse if follow-up is not divisible", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should NOT switch to mouse if follow-up comes after 10ms", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should call clearLinuxBuffer method after 10ms timeout", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should handle negative Linux wheel values", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should handle mixed sign Linux wheel values if divisible", () => {
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should allow buffering during 500ms cooldown as exception", () => {
      pointer.detectedDevice = "trackpad"
  ...
  describe("Performance and Efficiency", () => {
    test("should not create timers for regular wheel events", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
      pointer.detectedDevice = "mouse"
    ...
    test("should create at most one timer for Linux detection", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
      pointer.detectedDevice = "trackpad"
    ...
    test("should handle thousands of events efficiently", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
    ...
    test("should use minimal memory with timestamp approach", () => {
    ...
    test("should handle rapid mode switching efficiently", () => {
      const setTimeoutSpy = vi.spyOn(global, "setTimeout")
  ...
  describe("Edge Cases and Complex Scenarios", () => {
    test("should handle float values correctly for mouse detection", () => {
      pointer.detectedDevice = "trackpad"
      pointer.lastWheelEventTime = 0
      pointer.hasReceivedWheelEvent = true
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should handle integer values correctly for trackpad detection", () => {
      pointer.detectedDevice = "mouse"
      pointer.lastWheelEventTime = 0
      pointer.hasReceivedWheelEvent = true
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should correctly identify pinch-to-zoom with ctrlKey", () => {
      const event = new WheelEvent("wheel", {
        ctrlKey: true,
        deltaY: 250.5,
        deltaX: 0,
    ...
    test("should handle boundary values for pinch-to-zoom detection", () => {
    ...
    test("should handle boundary values for mouse wheel detection", () => {
      pointer.detectedDevice = "trackpad"
      pointer.lastWheelEventTime = 0
      pointer.hasReceivedWheelEvent = true
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should handle Linux wheel detection with various multiples", () => {
      pointer.detectedDevice = "trackpad"
      pointer.lastWheelEventTime = 0
      pointer.hasReceivedWheelEvent = true
      vi.spyOn(performance, "now").mockReturnValue(500)
    ...
    test("should not confuse trackpad integers with Linux wheel", () => {
      pointer.detectedDevice = "trackpad"
      pointer.lastWheelEventTime = 0
      pointer.hasReceivedWheelEvent = true
      vi.spyOn(performance, "now").mockReturnValue(500)
  ...
  describe("Input Type Validation", () => {
    describe("Two-finger panning validation", () => {
      test("should accept integer deltaY values", () => {
      ...
      test("should accept integer deltaX values", () => {
        const values = [0, 1, -1, 100, -100, 999, -999]
        for (const deltaX of values) {
          const event = new WheelEvent("wheel", {
            ctrlKey: false,
            deltaY: 5,
            deltaX,
      ...
      test("should handle ctrlKey true or false", () => {
    ...
    describe("Pinch-to-zoom validation", () => {
      test("should always have ctrlKey true", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: true,
          deltaY: 5.5,
          deltaX: 0,
      ...
      test("should accept float deltaY values in range -1000 to 1000", () => {
      ...
      test("should always have deltaX = 0", () => {
        const event = new WheelEvent("wheel", {
          ctrlKey: true,
          deltaY: 5.5,
          deltaX: 0,
    ...
    describe("Mouse input validation", () => {
      test("should accept float deltaX values in range -1000 to 1000", () => {
      ...
      test("should have deltaY >= 60 for Windows/Mac mouse", () => {
      ...
      test("should have integer deltaY as multiples of 10 or 15 for Linux", () => {
        // Base 10 multiples
    ...
    describe("Float vs Integer understanding", () => {
      ...
      test("should correctly validate pinch-to-zoom deltaY as float", () => {
