...
describe("LGraphNode", () => {
  ...
  describe("Disconnect I/O Slots", () => {
    test("should disconnect input correctly", () => {
      ...
      expect(graph.links.has(link?.id ?? -1)).toBe(false)
    ...
    test("should disconnect output correctly", () => {
      ...
      expect(graph.links.has(link1?.id ?? -1)).toBe(false)
      expect(graph.links.has(link2?.id ?? -1)).toBe(true)
      ...
      expect(graph.links.has(link2?.id ?? -1)).toBe(false)
      expect(graph.links.has(link4?.id ?? -1)).toBe(false)
