import type { ExportedSubgraph } from "@/litegraph"
...
describe("topologicalSortSubgraphs", () => {
  test("returns original order when there are no dependencies", () => {
  ...
  test("sorts leaf dependencies before their parents", () => {
  ...
  test("returns original order for a single subgraph", () => {
    const only = makeSubgraph("only")
    const result = topologicalSortSubgraphs([only])
    expect(result).toEqual([only])
  })

  test("returns original order for empty array", () => {
